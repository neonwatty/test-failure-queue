...
describe('Example Projects Integration Tests', () => {
  ...
  function runTfqCommand(projectPath: string, args: string[]): any {
    // Use a shell command that captures stderr to stdout and extracts JSON
    const shellCommand = `cd "${projectPath}" && TFQ_DB_PATH="${tempDbPath}" "${tfqBin}" run-tests ${args.join(' ')} --json 2>&1 | grep -o '{.*}$' || true`;
    
    let output: string;
    try {
      output = execSync(shellCommand, {
        encoding: 'utf8',
        shell: '/bin/bash'
      });
    } catch (error: any) {
      // Even if the command fails, we might have JSON output
      output = error.stdout || '';
      if (!output) {
        // Try without grep
        const fallbackCommand = `cd "${projectPath}" && TFQ_DB_PATH="${tempDbPath}" "${tfqBin}" run-tests ${args.join(' ')} --json 2>&1`;
        try {
          const fallbackOutput = execSync(fallbackCommand, {
            encoding: 'utf8',
            shell: '/bin/bash'
          });
          // Extract JSON from the fallback output
          const jsonMatch = fallbackOutput.match(/\{[\s\S]*\}$/);
          if (jsonMatch) {
            output = jsonMatch[0];
          } else {
            throw new Error(`No JSON output found in TFQ command output`);
          }
        } catch (fallbackError: any) {
          const fallbackOut = fallbackError.stdout || '';
          const jsonMatch = fallbackOut.match(/\{[\s\S]*\}$/);
          if (jsonMatch) {
            output = jsonMatch[0];
          } else {
            throw fallbackError;
    ...
    if (!output || output.trim() === '') {
      throw new Error('No JSON output from TFQ command');
    }
  ...
  describe('JavaScript Example (Jest)', () => {
    ...
    it('should detect exactly 2 failing test files with explicit parameters', () => {
      ...
      // Check that the failing test files are detected
  ...
  describe('TypeScript Example (Vitest)', () => {
    ...
    it('should detect exactly 2 failing test files', () => {
      ...
      // Check that the failing test files are detected
      const failingFiles = result.failingTests.map((f: string) => path.basename(f));
      expect(failingFiles).toContain('calculator.test.ts');
  ...
  describe('Ruby Example (Minitest)', () => {
    ...
    it('should detect exactly 2 failing test files', () => {
      ...
      const failingFiles = result.failingTests.map((f: string) => path.basename(f));
      expect(failingFiles).toContain('calculator_test.rb');
  ...
  describe('Rails Example (Minitest)', () => {
    ...
    it('should detect exactly 2 failing test files', () => {
      ...
      expect(failingFiles).toContain('test/controllers/users_controller_test.rb');
  ...
  describe('Python Example (Pytest)', () => {
    ...
    testOrSkip('should detect failing test files', () => {
